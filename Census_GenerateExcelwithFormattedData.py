...
##    Excel_File_Path = Excel_File_Path.replace("$!$","'")
    ...
    if stop_process == 0:
        ...
        server_name = 'prd-cdw.et1361.epichosted.com'
        database_name = 'CDW'
        ...
        if not payor_df.empty:
            single_payor_name = payor_df['PayorCategory'].iloc[0]        
        else:
            stop_process = 1
        ...
        payor_cell.alignment = Alignment(horizontal='center', vertical='top')  # Center alignment
        payor_cell.font = Font(name='Aptos Narrow', size=12, bold=True)
        
        
        location_cell = ws.cell(row=2, column=1, value=single_location_name)
        ws.merge_cells(start_row=2, start_column=1, end_row=2, end_column=total_columns)  # Merge cells
        location_cell.alignment = Alignment(horizontal='center', vertical='top')  # Center alignment
        ...
        for r_idx, row in enumerate(dataframe_to_rows(df, index=False, header=False), 4):  # Start from row 3
            for c_idx, value in enumerate(row, 1):
                ...
                cell.font = Font(name='Aptos Narrow', size=8)
