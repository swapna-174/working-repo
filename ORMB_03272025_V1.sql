...
WITH BenefitPlanGroupers AS(
  ...
  FROM CoverageDim  left  join  CoverageDim CoverageDim_epp 
 /***  SB Added  the Join Condition to use the BenefitPlanEpicId ***/
  ...
  WHERE Details.BillingType <> 'CLAIM'
)
--SELECT * From BenefitPlanGroupers
...
,PlanBillingWindowDates AS (
  SELECT
    *
    ,CASE WHEN WithoutLookback.LookbackWindow = '1 Month' AND WithoutLookback.BillingWindowEnd LIKE '%15%' THEN DATEADD(MONTH, DATEDIFF(MONTH, 0, WithoutLookback.BillingWindowEndDate)-1, 0) + 15
          WHEN WithoutLookback.LookbackWindow = '1 Month' THEN DATEADD(MONTH, DATEDIFF(MONTH, 0, WithoutLookback.BillingWindowEndDate), 0)
          WHEN WithoutLookback.LookbackWindow = '2 Months' AND WithoutLookback.BillingWindowEnd LIKE '%15%' THEN DATEADD(MONTH, DATEDIFF(MONTH, 0, WithoutLookback.BillingWindowEndDate)-2, 0) + 15
          ...
          WHEN WithoutLookback.LookbackWindow = '45 Days' THEN DATEADD(MONTH, DATEDIFF(MONTH, 0, WithoutLookback.BillingWindowEndDate)-1, 0) + 15
          WHEN WithoutLookback.LookbackWindow = 'Modified 2 Months' AND WithoutLookback.BillingWindowEnd LIKE '%15%' THEN DATEADD(MONTH, DATEDIFF(MONTH, 0, WithoutLookback.BillingWindowEndDate)-2, 0) + 15
  ...
  FROM (
    ...
    FROM BenefitPlanGroupers
    LEFT OUTER JOIN DatesGrid
      ON DatesGrid.RunDate = @RunDate
    ) WithoutLookback
  )
 ---select *from PlanBillingWindowDates where  BenefitPlanKey=196189
, LatestCompletedVisitInRange AS (
  SELECT
    VisitFact.PatientDurableKey
    ,MAX(DateDim.DateValue) AS VisitDate
  FROM VisitFact
  LEFT OUTER JOIN DateDim
     ON DateDim.DateKey = VisitFact.AppointmentDateKey
  LEFT OUTER JOIN PatientExternalCoverageFactX
    ON PatientExternalCoverageFactX.PatientDurableKey = VisitFact.PatientDurableKey
  LEFT OUTER JOIN CoveragePatientMappingFactX  ON CoveragePatientMappingFactX.PatientDurableDurableKey = VisitFact.PatientDurableKey
  LEFT OUTER JOIN BenefitPlanGroupers CoverageDim_Cvg ON CoverageDim_Cvg.BenefitPlanKey = COALESCE(PatientExternalCoverageFactX.ExternalCoverageKey,CoveragePatientMappingFactX.CoverageKey)
  LEFT OUTER JOIN PlanBillingWindowDates 
		ON PlanBillingWindowDates.BenefitPlanKey =CoverageDim_Cvg.BenefitPlanKey
  WHERE VisitFact.AppointmentStatus  IN ('Completed')	--('Arrived','Completed')
	-- Checking if DOS falls withinn Billing Lookback Period ---
AND DateDim.DateValue >= PlanBillingWindowDates.LookbackStartDate
 AND DateDim.DateValue <= PlanBillingWindowDates.BillingWindowEndDate
  GROUP BY VisitFact.PatientDurableKey
),
...
InitialResults AS (
  SELECT
    ...
    ,[Health Plan Patient Id]		= Coalesce(PatientExternalCoverageFactX .ExternalCoverageSubscriberNumber,CoveragePatientMappingFactX.PatientSubscriberNumber )
    ...
    ,[Primary Insurance Number]		= Coalesce(PatientExternalCoverageFactX .ExternalCoverageSubscriberNumber,CoveragePatientMappingFactX.PatientSubscriberNumber )
    ...
    ,[Primary Contract]				= CONCAT(ExtCoverageDim.BenefitPlanName,' ',PatientDim.City,' ',ExtCoverageDim.PayorFinancialClass)
  ...
  LEFT OUTER JOIN LatestCompletedVisitInRange
    ON LatestCompletedVisitInRange.PatientDurableKey = SocialCareEpisodeFact.PrimaryPatientDurableKey
  INNER JOIN PatientDim
    ON PatientDim.DurableKey = SocialCareEpisodeFact.PrimaryPatientDurableKey
    AND PatientDim.IsCurrent = 1
  LEFT OUTER JOIN PatientExternalCoverageFactX
    ON PatientExternalCoverageFactX.PatientDurableKey = SocialCareEpisodeFact.PrimaryPatientDurableKey
  ...
  LEFT OUTER JOIN CoveragePatientMappingFactX
    ON CoveragePatientMappingFactX.PatientDurableDurableKey = PatientDim.DurableKey
	 LEFT OUTER JOIN PlanBillingWindowDates
    ON PlanBillingWindowDates.BenefitPlanKey = Coalesce(PatientExternalCoverageFactX.ExternalCoverageKey, CoveragePatientMappingFactX.CoverageKey)
	 LEFT OUTER JOIN CoverageDim ExtCoverageDim
    ON ExtCoverageDim.CoverageKey = Coalesce(PatientExternalCoverageFactX.ExternalCoverageKey, CoveragePatientMappingFactX.CoverageKey)
  WHERE SocialCareEpisodeFact.SocialCareEpisodeKey > 0  --Actual Episodes
    AND SocialCareEpisodeFact.EnrollmentDate <= PlanBillingWindowDates.BillingWindowEndDate  --Enrolled before end of billing window
    AND (SocialCareEpisodeFact.EpisodeEndDate >= PlanBillingWindowDates.BillingWindowStartDate OR SocialCareEpisodeFact.EpisodeEndDate IS NULL)  --Discharged after billing window starts or still active
    AND SocialCareEpisodeFact.EnrollmentDate IS NOT NULL  -- Was at one point on service
    AND PlanBillingWindowDates.BillingType IN( 'ORMB INVOICE', 'ORMB INVOICE AND CLAIM')
    --AND PlanBillingWindowDates.BenefitPlanName LIKE '%Anthem%'
...
SELECT  
  ...
  ,[Date Reinstated]				
  ,[Last In Person Assessment Date]
...
FROM InitialResults 
WHERE RN = 1 
  AND ([Discharge Date] >= BillingWindowEndDate OR [Discharge Date] IS NULL OR ([Date Reinstated] IS NOT NULL AND [Date Reinstated]	<= BillingWindowEndDate))
  ---testing Durablekey-318587
 --- order by [Primary Insurance Name]  asc